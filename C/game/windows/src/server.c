...
CRITICAL_SECTION cs; // クリティカルセクション制御用
...
// 通信はここでする(マルチスレッド)
unsigned __stdcall command_process(void *so)
{
	...
	while (1)
	{
		...
		{
			strcpy_s(buffer, BUFSIZ, "衝突しました\n引き分け");
			send(s, buffer, BUFSIZ, 0);
			printf("ccount=%d\n", connectioncount);
			gamestats = 0;
			connectioncount--;
			game_clear(player.pid);
	...
	mes_send:
		if (SOCKET_ERROR == send(s, buffer, BUFSIZ, 0)) // 送信
			goto finally;									// 送信に問題があった場合finallyへ移行
	...
	if (SOCKET_ERROR == closesocket(s))		   // sとの接続を切断する
...
int main()
{
	...
	SOCKADDR_IN srcAddr;			// sockaddr_in 構造体
	...
	while (1)
	{
		...
		*s = accept(listen_s, (struct sockaddr *)&srcAddr, &len);
		...
		if (INVALID_SOCKET != *s && connectioncount < 2)
		{
			...
			id++;
			HANDLE th;
			th = (HANDLE)_beginthreadex(NULL, 0, command_process, (void *)s, 0, NULL); // スレッドを立てる
			if (th)
		...
		else if (connectioncount >= 2)
		{
			...
			char buffer[BUFSIZ];
			strcpy_s(buffer, BUFSIZ, "このサーバには同時に2人までしか接続できません");
			send(*s, buffer, BUFSIZ, 0);
			//send(*s, '\0', BUFSIZ, 0);
			closesocket(*s);
		}
	...
	// Windows での終了設定
	DeleteCriticalSection(&cs); // クリティカルセクション削除
